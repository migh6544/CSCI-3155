...
Go through the below code and answer the following questions:
  ...
  1. Which case will be matched for the given list "myList" ?
  2. Add an additional case for extracting the two elements present in the list.
...
myList match {
    ...
    // Begin solution
    case a :: b :: Nil => s"List contains two elements $a and $b"
    // End solution
...
// Exercise: 2
// exp3 = (5 * (4 + 10)) - (4 / 2)
val exp3: Expr = Binary(Minus, Binary(Times, Num(5), Binary(Plus, Num(4), Num(10))), Binary(Div, Num(4), Num(2)))
...
def getConstants(e: Expr): List[Double] = {
  ...
  e match {
    case Num(n) => n :: Nil
    case Unary(_, e) => getConstants(e)
    case Binary(_, e1, e2) => getConstants(e1) ++ getConstants(e2)
  }
  // End Solution
}
